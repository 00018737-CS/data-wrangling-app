...
def render():
    ...
    with st.container(border=True):
        st.write("### 🎛️ Scaling Settings")
        
        # User selects columns (Requirement: allow user to choose columns)
        ...
        # Scaling method selection (Requirement: offer at least two methods)
        method = st.radio(
        ...
        # SMART PREVIEW: BEFORE AND AFTER STATS
        c1, c2 = st.columns(2)
        
        # 1. Calculate and show current statistics
        with c1:
            st.write("📊 **Stats BEFORE:**")
            st.dataframe(df[selected_cols].describe().T[['mean', 'std', 'min', 'max']], use_container_width=True)

        # 2. Virtually apply formulas for the preview
        ...
        # 3. Show resulting statistics for the user to review
        with c2:
