...
def render():
    ...
    with st.container(border=True):
        ...
        # Mask all rows involved in duplication (keep=False) for preview
        all_duplicates_mask = df.duplicated(subset=subset_cols, keep=False)
        
        # Mask only the redundant copies (keep='first')
    ...
    # 2. DUPLICATE GROUPS PREVIEW
    with st.container(border=True):
        st.write("### 👁️ 2. Duplicate Groups Preview")
    ...
    with st.container(border=True):
        st.write("### 🛠️ 3. Resolution Actions")
        ...
        # Map user choice to Pandas arguments
        if "Keep First" in keep_action:
            keep_val = 'first'
        elif "Keep Last" in keep_action:
            keep_val = 'last'
        else:
            keep_val = False # Drops all instances

        # Recalculate potential data loss based on action
