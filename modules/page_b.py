...
# Required for AI functionality: pip install google-generativeai
import google.generativeai as genai

# Import manual cleaning modules
...
def render():
    ...
    # AI ASSISTANT (11 SAFE OPERATIONS)
    st.write("---")
    ai_enabled = st.toggle("✨ Enable AI Assistant (Natural Language Cleaning)")
    
    if ai_enabled:
        with st.container(border=True):
            st.caption("⚠️ **Disclaimer:** AI outputs may be imperfect. Always review the suggested operations before approving.")
            ...
            if 'ai_suggestions' in st.session_state:
                ...
                for action in st.session_state.ai_suggestions:
                    ...
                    if act == "drop_duplicates":
                        st.info("🗑️ **Drop Duplicates:** Completely identical rows will be removed.")
                        ...
                        st.info(f"✂️ **Drop Columns:** Columns `{', '.join(action.get('columns', []))}` will be permanently removed.")
                    elif act == "drop_empty_columns":
                        st.info(f"🌪️ **Drop Empty Columns:** Columns with more than `{action.get('threshold_pct')}%` missing values will be dropped.")
                    elif act == "rename_column":
                        st.info(f"🏷️ **Rename:** Column `{action.get('old_name')}` will be renamed to `{action.get('new_name')}`.")
                    ...
                    if st.button("✅ Yes, Approve & Apply", use_container_width=True):
                        ...
                        for action in st.session_state.ai_suggestions:
                            ...
                            try:
                                ...
                                # Ignore single action errors to prevent app crash
